...
import { restrictedSpineContentReason, restrictedSpinePathReason } from "./distribution-policy.mjs";
...
const restrictedRepositoryFiles = repositoryFiles.filter((path) => restrictedSpinePathReason(path));
...
for (const path of repositoryFiles) {
  const reason = restrictedSpineContentReason(await readFile(resolve(root, path)));
...
const walkPublishable = async (path, relativePath) => {
  ...
  const reason = restrictedSpineContentReason(bytes);
...
const restrictedPublishableFiles = publishableFiles.filter((path) => restrictedSpinePathReason(path));
